...
struct Renderer {
public:
	Renderer(unsigned int width, unsigned int height, unsigned int channels, unsigned int samples_per_pixel, unsigned int accumulation_limit, unsigned int max_path_length, SceneSelector SELECTED_SCENE);
    ~Renderer();

    static void Renderer::handle_error(void* userPtr, const RTCError code, const char* str);
    ...
    void Renderer::render_tile_task(int taskIndex, int threadIndex, const int numTilesX,
        const int numTilesY, RandomEngine& reng,
        std::uniform_real_distribution<float>& distrib);

    Vec3fa Renderer::render_pixel_samples(int x, int y, RandomEngine& reng,
        std::uniform_real_distribution<float>& distrib);
...
Renderer::Renderer(unsigned int width, unsigned int height, unsigned int channels, unsigned int samples_per_pixel, unsigned int accumulation_limit, unsigned int max_path_length, SceneSelector SELECTED_SCENE) : m_width(width), m_height(height), m_channels(channels), m_spp(samples_per_pixel), m_accu_limit(accumulation_limit), m_max_path_length(max_path_length), m_sceneSelector(SELECTED_SCENE) {
    m_accu_count = 0;
