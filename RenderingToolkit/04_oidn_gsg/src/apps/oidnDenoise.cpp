...
void printUsage() {
  ...
  std::cout << "usage: oidnDenoise [-f/--filter RT|RTLightmap]" << std::endl
            << "                   [--hdr color.pfm] [--ldr color.pfm] "
               "[--srgb] [--dir directional.pfm]"
            << std::endl
            << "                   [--alb albedo.pfm] [--nrm normal.pfm] "
               "[--clean_aux]"
            << std::endl
            << "                   [--is/--input_scale value]" << std::endl
            << "                   [-o/--output output.pfm] [-r/--ref "
               "reference_output.pfm]"
            << std::endl
            << "                   [-w/--weights weights.tza]" << std::endl
            << "                   [--threads n] [--affinity 0|1] [--maxmem "
               "MB] [--inplace]"
            << std::endl
...
void signalHandler(int signal) { isCancelled = true; }
...
std::vector<char> loadFile(const std::string& filename) {
  ...
  if (file.fail()) throw std::runtime_error("cannot open file: " + filename);
  file.seekg(0, file.end);
  const size_t size = file.tellg();
  file.seekg(0, file.beg);
  std::vector<char> buffer(size);
  file.read(buffer.data(), size);
  if (file.fail()) throw std::runtime_error("error reading from file");
  return buffer;
}
...
int main(int argc, char* argv[]) {
  ...
  try {
    ...
    while (args.hasNext()) {
      ...
      else if (opt == "is" || opt == "input_scale" || opt == "inputScale" ||
               opt == "inputscale")
    ...
    if (!refFilename.empty() && numBenchmarkRuns > 0)
      throw std::runtime_error(
          "reference and benchmark modes cannot be enabled at the same time");

#if defined(OIDN_X64)
    // Set MXCSR flags
    if (!refFilename.empty()) {
      // In reference mode we have to disable the FTZ and DAZ flags to get
      // accurate results
    ...
    if (!normalFilename.empty()) input = normal = loadImage(normalFilename, 3);

    if (!colorFilename.empty())
      input = color = loadImage(colorFilename, 3, srgb);

    if (!input) throw std::runtime_error("no input image specified");
    ...
    if (setAffinity >= 0) device.set("setAffinity", bool(setAffinity));
    ...
    if (color)
      filter.setImage("color", color->data(), Format::Float3, color->width,
                      color->height);
    if (albedo)
      filter.setImage("albedo", albedo->data(), Format::Float3, albedo->width,
                      albedo->height);
    if (normal)
      filter.setImage("normal", normal->data(), Format::Float3, normal->width,
                      normal->height);

    filter.setImage("output", output->data(), Format::Float3, output->width,
                    output->height);

    if (filterType == "RT") {
      if (hdr) filter.set("hdr", true);
      if (srgb) filter.set("srgb", true);
    } else if (filterType == "RTLightmap") {
      if (directional) filter.set("directional", true);
    }

    if (std::isfinite(inputScale)) filter.set("inputScale", inputScale);

    if (cleanAux) filter.set("cleanAux", cleanAux);

    if (maxMemoryMB >= 0) filter.set("maxMemoryMB", maxMemoryMB);
    ...
    std::cout << "  device=CPU"
              ...
              << versionPatch << ", msec=" << (1000. * deviceInitTime)
              << std::endl
              << "  filter=" << filterType
              << ", msec=" << (1000. * filterInitTime) << std::endl;

    // Denoise the image
    if (!showProgress) std::cout << "Denoising" << std::endl;
    timer.reset();
    ...
    if (ref) {
      ...
      std::cout << "  values=" << output->size() << ", errors=" << numErrors
    ...
    if (numBenchmarkRuns > 0) {
      ...
      std::cout << "Benchmarking: "
                << "ntimes=" << numBenchmarkRuns << std::endl;
      timer.reset();

      for (int i = 0; i < numBenchmarkRuns; ++i) filter.execute();

      const double totalTime = timer.query();
      std::cout << "  sec=" << totalTime
