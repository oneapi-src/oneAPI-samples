...
int main(int argc, char **argv)
{
    ...
    const std::size_t bytes = desired_total_size * sizeof(int);

    std::cout << "memory footprint = " << 3 * bytes * 1E-9 << " GB"
              << std::endl;

    int *a = static_cast<int*>(omp_target_alloc_device(bytes, device_id));
    int *b = static_cast<int*>(omp_target_alloc_device(bytes, device_id));
    int *c = static_cast<int*>(omp_target_alloc_device(bytes, device_id));
            ...
            for (int lz = min; lz < max; lz *= 2)
            {
                ...
                for (int irep = 0; irep < no_max_rep + warmup; ++irep)
                {
                    ...
                    #pragma omp target teams distribute parallel for \
                        simd simdlen(32) thread_limit(64) collapse(3)
                ...
                const int64_t streamed_bytes = 3 * total_size * sizeof(int);
