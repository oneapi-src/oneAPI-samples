...
// =============================================================
//
// The STREAM sample works as a memory bandwidth benchmark.
//
 ...
 *         systems).  Use of non-zero values for OFFSET can be especially
 *         helpful if the STREAM_ARRAY_SIZE is set to a value close to a large
 *         power of 2.
 ...
 *       OMP_NUM_THREADS allows runtime control of the number of threads/cores
 *       used when the resulting "stream_omp" program is executed.
 *
 ...
 *       the code to call separate functions to execute each kernel.  Trivial
 *       versions of these functions are provided, but they are *not* tuned --
 *       they just provide predefined interfaces to be replaced with tuned code.
 *
 *-----------------------------------------------------------------------*/
...
int main(void) {
  ...
  std::cerr << "SYCL Platform: " << p.get_info<info::platform::name>()
            << "\n";
  std::cerr << "SYCL Device:   " << d.get_info<info::device::name>()
            << "\n";
...
int checktick() {
  ...
  // Determine the minimum difference between these M values.  This result will
  // be our estimate (in microseconds) for the clock granularity.
  ...
  if (abs(aAvgErr / aj) > epsilon) {
    ...
    for (j = 0; j < STREAM_ARRAY_SIZE; j++) {
      if (abs(a[j] / aj - 1.0) > epsilon) {
        ...
        if (ierr < 10) {
          printf(
              "         array a: index: %ld, expected: %e, observed: %e, "
              "relative error: %e\n",
  ...
  if (abs(bAvgErr / bj) > epsilon) {
    ...
    for (j = 0; j < STREAM_ARRAY_SIZE; j++) {
      if (abs(b[j] / bj - 1.0) > epsilon) {
        ...
        if (ierr < 10) {
          printf(
              "         array b: index: %ld, expected: %e, observed: %e, "
              "relative error: %e\n",
  ...
  if (abs(cAvgErr / cj) > epsilon) {
    ...
    for (j = 0; j < STREAM_ARRAY_SIZE; j++) {
      if (abs(c[j] / cj - 1.0) > epsilon) {
        ...
        if (ierr < 10) {
          printf(
              "         array c: index: %ld, expected: %e, observed: %e, "
              "relative error: %e\n",
...
void tuned_STREAM_Copy() {
  q.parallel_for(range{STREAM_ARRAY_SIZE},
                 [=, a = a, c = c](item<1> i) {
                   const auto j = i[0];
                   c[j] = a[j];
                 });
