...
## Purpose
This sample code shows how to get started with scaling out the training of a neural network in TensorFlow on multiple compute nodes in a cluster. The sample uses Horovod* as a framework that facilitates the task of distributing the workload. Horovod's  core principles are based on MPI concepts such as size, rank, local rank, allreduce, allgather and, broadcast.
...
```
    mpirun -n 2 TensorFlow_Multinode_Training_with_Horovod.py
```
...
```
    mpirun -n 2 -hosts host1,host2 TensorFlow_Multinode_Training_with_Horovod.py
```
