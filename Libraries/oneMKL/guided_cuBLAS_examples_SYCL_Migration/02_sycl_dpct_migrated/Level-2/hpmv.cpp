...
int main(int argc, char *argv[]) try {
  ...
  const std::vector<data_type> AP = {
      {1.1, 1.2}, {3.5, 3.6}, {2.3, 2.4}, {4.7, 4.8}};
  ...
  d_AP = (data_type *)sycl::malloc_device(sizeof(data_type) * AP.size(), q_ct1);
  d_x = (data_type *)sycl::malloc_device(sizeof(data_type) * x.size(), q_ct1);
  d_y = (data_type *)sycl::malloc_device(sizeof(data_type) * y.size(), q_ct1);

  stream->memcpy(d_AP, AP.data(), sizeof(data_type) * AP.size());
  stream->memcpy(d_x, x.data(), sizeof(data_type) * x.size());

  /* step 3: compute */
  oneapi::mkl::blas::column_major::hpmv(
      *cublasH, uplo, n, std::complex<double>(alpha.x(), alpha.y()),
      (std::complex<double> *)d_AP, (std::complex<double> *)d_x, incx,
      std::complex<double>(beta.x(), beta.y()), (std::complex<double> *)d_y,
      incy);

  /* step 4: copy data to host */
  stream->memcpy(y.data(), d_y, sizeof(data_type) * y.size());
  stream->wait();
