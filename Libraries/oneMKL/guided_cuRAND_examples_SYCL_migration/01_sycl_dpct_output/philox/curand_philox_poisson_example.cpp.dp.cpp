...
void run_on_device(const int &n, const unsigned long long &offset,
                   const unsigned long long &seed, const double &lambda,
  ...
  CURAND_CHECK(
      DPCT_CHECK_ERROR(gen = dpct::rng::create_host_rng(
  ...
  /* Set ordering */
  /*
  DPCT1007:8: Migration of curandSetGeneratorOrdering is not supported.
  */
...
int main(int argc, char *argv[]) try {
  ...
  dpct::rng::random_engine_type rng =
      dpct::rng::random_engine_type::philox4x32x10;
  ...
  /* Create stream */
  /*
  DPCT1025:10: The SYCL queue is created ignoring the flag and priority options.
  */
  ...
  run_on_host(n, offset, seed, lambda, order, rng, stream, gen, h_data);

  printf("Host\n");
  print_vector(h_data);
  printf("=====\n");

  run_on_device(n, offset, seed, lambda, order, rng, stream, gen, h_data);
