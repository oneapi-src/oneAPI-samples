...
void run_on_host(const int &n, const unsigned long long &offset,
  ...
  /* Set ordering */
  /*
  DPCT1007:12: Migration of curandSetGeneratorOrdering is not supported.
  */
  CURAND_CHECK(curandSetGeneratorOrdering(gen, order));
...
int main(int argc, char *argv[]) try {
  ...
  */
  ...
  run_on_host(n, offset, seed, order, rng, stream, gen, h_data);

  printf("Host\n");
  print_vector(h_data);
  printf("=====\n");

  run_on_device(n, offset, seed, order, rng, stream, gen, h_data);
