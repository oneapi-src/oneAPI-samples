...
// T-test threshold which corresponds to 5% significance level and infinite degrees of freedom
static const auto threshold = 1.95996f;

// T-test function with expected mean
// Returns: -1 if something went wrong, 1 - in case of NULL hypothesis should be accepted, 0 - in case of NULL hypothesis should be rejected
template <typename RealType>
std::int32_t t_test(sycl::queue& queue, sycl::buffer<RealType, 1>& r, std::int64_t n, RealType expected_mean) {
    ...
    auto dataset = oneapi::mkl::stats::make_dataset<oneapi::mkl::stats::layout::row_major>(1, n, r);
    ...
    auto variance_acc = variance_buf.template get_access<sycl::access::mode::read>();
    if ((sycl::abs(mean_acc[0] - expected_mean) * sqrt_n_observations / sycl::sqrt(variance_acc[0])) < static_cast<RealType>(threshold)) {
        res = 1;
    } else {
        res = 0;
    }
...
// Returns: -1 if something went wrong, 1 - in case of NULL hypothesis should be accepted, 0 - in case of NULL hypothesis should be rejected
template <typename RealType>
std::int32_t t_test(sycl::queue& queue, sycl::buffer<RealType, 1>& r1, std::int64_t n1, sycl::buffer<RealType, 1>& r2, std::int64_t n2) {
    ...
    // Perform computations of mean and variance
    auto dataset1 = oneapi::mkl::stats::make_dataset<oneapi::mkl::stats::layout::row_major>(1, n1, r1);
    auto dataset2 = oneapi::mkl::stats::make_dataset<oneapi::mkl::stats::layout::row_major>(1, n2, r2);
    ...
    auto variance1_acc = variance1_buf.template get_access<sycl::access::mode::read>();
    auto mean2_acc = mean2_buf.template get_access<sycl::access::mode::read>();
    auto variance2_acc = variance2_buf.template get_access<sycl::access::mode::read>();
    bool almost_equal = (variance1_acc[0] < 2 * variance2_acc[0]) || (variance2_acc[0] < 2 * variance1_acc[0]);
    if (almost_equal) {
        if ((sycl::abs(mean1_acc[0] - mean2_acc[0]) / sycl::sqrt((static_cast<RealType>(1.0) / static_cast<RealType>(n1) +
            static_cast<RealType>(1.0) / static_cast<RealType>(n2)) *((n1 - 1) * (n1 - 1) * variance1_acc[0] + (n2 - 1) *
            (n2 -1) * variance2_acc[0]) / (n1 + n2 - 2))) < static_cast<RealType>(threshold)) {
            res = 1;
        } else {
            res = 0;
        }
    } else {
        if ((sycl::abs(mean1_acc[0] - mean2_acc[0]) / sycl::sqrt((variance1_acc[0] + variance2_acc[0]))) < static_cast<RealType>(threshold)) {
            res = 1;
        } else {
            res = 0;
        }
