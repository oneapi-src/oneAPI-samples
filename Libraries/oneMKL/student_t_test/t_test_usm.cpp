...
std::int32_t t_test(sycl::queue& queue, RealType* r, std::int64_t n, RealType expected_mean) {
    ...
    queue.wait_and_throw();
    // Check the condition
    if ((sycl::abs(mean[0] - expected_mean) * sqrt_n_observations / sycl::sqrt(variance[0])) < static_cast<RealType>(threshold)) {
        res = 1;
    } else {
        res = 0;
    }
...
// Returns: -1 if something went wrong, 1 - in case of NULL hypothesis should be accepted, 0 - in case of NULL hypothesis should be rejected
template <typename RealType>
std::int32_t t_test(sycl::queue& queue, RealType* r1, std::int64_t n1, RealType* r2, std::int64_t n2) {
    ...
    queue.wait_and_throw();
    // Check the condition
    bool almost_equal = (variance1[0] < 2 * variance2[0]) || (variance2[0] < 2 * variance1[0]);
    if (almost_equal) {
        if ((sycl::abs(mean1[0] - mean2[0]) / sycl::sqrt((static_cast<RealType>(1.0) / static_cast<RealType>(n1) +
            static_cast<RealType>(1.0) / static_cast<RealType>(n2)) *((n1 - 1) * (n1 - 1) * variance1[0] + (n2 - 1) *
            (n2 -1) * variance2[0]) / (n1 + n2 - 2))) < static_cast<RealType>(threshold)) {
            res = 1;
        } else {
            res = 0;
        }
    } else {
        if ((sycl::abs(mean1[0] - mean2[0]) / sycl::sqrt((variance1[0] + variance2[0]))) < static_cast<RealType>(threshold)) {
            res = 1;
        } else {
            res = 0;
        }
