...
)
{
    ...
    double N_d1 = 1. / 2. + 1. / 2. * std::erf(((log(S / L) + (r + 0.5 * sigma * sigma) * t) / (sigma * std::sqrt(t))) / std::sqrt(2.));
    double N_d2 = 1. / 2. + 1. / 2. * std::erf(((log(S / L) + (r - 0.5 * sigma * sigma) * t) / (sigma * std::sqrt(t))) / std::sqrt(2.));
...
void Binomial::check()
{
    if (VERBOSE) {
        ...
        for (int opt = 0; opt < opt_n; opt++)
            BlackScholesRefImpl(h_call_result_host[opt], h_stock_price[opt], h_option_strike[opt], h_option_years[opt], risk_free, volatility);
