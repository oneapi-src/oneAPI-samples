...
void Binomial::body() {
    ...
    binomial_queue->submit([&] (sycl::handler& h) { {
            ...
            h.template parallel_for(sycl::nd_range(sycl::range<1>(opt_n * wg_size), sycl::range<1>(wg_size)),
                [=](sycl::nd_item<1> item) [[intel::kernel_args_restrict]] [[intel::reqd_sub_group_size(sg_size)]] {{
                        ...
                        for (int i = 0; i < block_size; i++) {
                            DATA_TYPE d = sx * sycl::exp(id) - xx;
                            local_call[i] = (d > 0) ? d : 0;
                            id += mul_c;
                        }

                        // Handling num_steps step by last item and putting it direclty to SLM last element
                        ...
                        for (int i = num_steps; i > 0; i--) {
                            // Give and get "next block's local_call[j+1]" (local_call[0] in next block) elements across work items
                            ...
                            if (block_start <= i) {
                                for (int j = 0; j < block_size; j++) {
                                    local_call[j] = pu_df * local_call[j + 1] + pd_df * local_call[j];
                                }
                            }
                        }
                        if (local_id == 0){
...
void Binomial::run()
{
    std::printf("%s Precision Binomial Option Pricing version %d.%d running on %s using DPC++, workgroup size %d, sub-group size %d.\n",
        sizeof(DATA_TYPE) > 4 ? "Double" : "Single", MAJOR, MINOR, binomial_queue->get_device().get_info<sycl::info::device::name>().c_str(), wg_size, sg_size);

    std::printf("Compiler Version: %s, LLVM %d.%d based.\n", __VERSION__, __clang_major__, __clang_minor__);
    std::printf("Driver Version  : %s\n", binomial_queue->get_device().get_info<sycl::info::device::driver_version>().c_str());
