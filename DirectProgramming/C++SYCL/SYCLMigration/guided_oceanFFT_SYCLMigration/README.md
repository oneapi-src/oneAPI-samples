...
| Software              | SYCLomatic (Tag - 20241104) <br> Intel® oneAPI Base Toolkit (Base Kit) version 2025.0 <br> oneAPI for NVIDIA GPU plugin from Codeplay (to run SYCL™ applications on NVIDIA® GPUs)
...
1. Few CUDA headers are not migrated to SYCL which contain some macros which are used in code. It has been changed manually.
      ...
      ```
2. opensource oneMKL has been changed into oneMath, need to adjust the code for Nvidia device support
      ```
      #ifdef NVIDIA_GPU
      ...
      #endif
      ```
      `oneapi::mkl` namespace has changed into `oneapi::math` for opensource oneMKL.
