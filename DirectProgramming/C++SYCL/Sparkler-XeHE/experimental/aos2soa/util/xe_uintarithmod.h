...
namespace xehe
{
    ...
    {
        template<typename T>
        void increment_uint_mod(
            const T *operand, const T *modulus, std::size_t uint64_count, T *result)
        {
#ifdef XeHE_DEBUG
            if (!operand)
            {
            ...
            const T *operand, const T *modulus, std::size_t uint64_count, T *result)
        {
            ...
            if (*operand & 1)
            {
                ...
                if (carry)
                {
                    set_bit_uint_uint(result, uint64_count, static_cast<int>(uint64_count) * bits_per_uint64 - 1);
                }
            }
            else
            {
        ...
        void add_uint_uint_mod(
            const T *operand1, const T *operand2, const T *modulus,
