...
namespace xehe
{
    namespace util
    {
            ...
            T* value, std::size_t uint64_count)
        {
            ...
            size_t nibbles = (sizeof(T) == 8) ? nibbles_per_uint64 : nibbles_per_uint32;
            size_t num_nibbles =
                mul_safe(uint64_count, nibbles);
            ...
            for (size_t i = 0; i < uint64_count; i++)
            {
                ...
                for (size_t j = 0; j < num_nibbles; j++)
                {
                    size_t nibble = safe_cast<size_t>(part & T(0x0F));
            ...
            T* value, std::size_t uint64_count)
        {
            ...
            auto remainder = allocate_uint<T>(uint64_count);
            auto quotient = allocate_uint<T>(uint64_count);
            auto base = allocate_uint<T>(uint64_count);
            auto *remainderptr = remainder.get();
            auto *quotientptr = quotient.get();
            ...
            while (!is_zero_uint(remainderptr, uint64_count))
            {
                divide_uint_uint_inplace(remainderptr, baseptr,
                    uint64_count, quotientptr);
                char digit = static_cast<char>(
                    remainderptr[0] + static_cast<T>('0'));
                output += digit;
