...
namespace seal
{
    ...
    void xehe_copy_ciphertext(Ciphertext &self, Ciphertext &assign){
        // Copy over fields
        self.parms_id_ = assign.parms_id_;
        self.is_ntt_form_ = assign.is_ntt_form_;
        self.scale_ = assign.scale_;

        self.resize_internal(assign.size_, assign.poly_modulus_degree_, assign.coeff_modulus_size_, true);
        self.get_gpu_cipher() = assign.get_gpu_cipher();
 
        if (!assign.on_gpu() || assign.get_gpu_mem() == nullptr || (assign.get_gpu_mem() != nullptr && assign.is_dirty()))        
        {
             copy(assign.data_.cbegin(), assign.data_.cend(), self.data_.begin());
        }
 
    }

    Ciphertext::Ciphertext(const Ciphertext &assign)
    {
        *this = assign;
    }

    void Ciphertext::download(void)
    {
 
        auto download_sz = dyn_array().size();
        get_gpu_cipher().download((void *)data(), download_sz);

    }

    void Ciphertext::upload(void)
    {
        auto upload_sz = dyn_array().size();
        get_gpu_cipher().upload((const void *)data(), upload_sz);
    }
...
#ifdef SEAL_USE_INTEL_XEHE
        xehe_copy_ciphertext(*this, (Ciphertext &)assign);
#else
...
#ifdef SEAL_USE_INTEL_XEHE
        auto new_size = dyn_array().size();
        resize_gpu_internal(new_size);
#endif
    }
...
#ifdef SEAL_USE_INTEL_XEHE
        auto new_size = dyn_array().size();
        resize_gpu_internal(new_size, discard_alloc);
#endif
    }
    ...
    void Ciphertext::load_members(const SEALContext &context, istream &stream, SEAL_MAYBE_UNUSED SEALVersion version)
    {
        ...
        {
		    get_gpu_cipher().set_prime64(context.is_prime64());
            set_dirty(true);
            upload();
        }
#endif   			        
