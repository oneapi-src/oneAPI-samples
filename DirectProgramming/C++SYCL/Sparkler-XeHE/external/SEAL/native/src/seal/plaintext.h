    ...
    */
    ...
    public:
        ...
        */
        ...
        const xehe::plgin::XeHECiphertext &get_gpu_plain(void) const
        {
            return (gpu_plain_);
        }

        xehe::plgin::XeHECiphertext &get_gpu_plain(void)
        {
            return (gpu_plain_);
        }
        ...
        {
            ((xehe::plgin::XeHECiphertext &)get_gpu_plain()).set_prime64(val);
        }

        inline bool is_prime64(void) const
        {
            return(get_gpu_plain().is_prime64());
        }

        inline void set_dirty(bool val)
        {
            ((xehe::plgin::XeHECiphertext &)get_gpu_plain()).set_dirty(val);
        } 

        inline bool is_dirty(void) const 
        {
             return (get_gpu_plain().is_dirty());
        }

        const std::shared_ptr<xehe::ext::Buffer<uint64_t>>& get_gpu_mem(void) const
        {
            return (get_gpu_plain().get_gpu_mem<uint64_t>()); //(gpu_mem_);
        }

        std::shared_ptr<xehe::ext::Buffer<uint64_t>>& get_gpu_mem(void)
        {
            return (((xehe::plgin::XeHECiphertext &)get_gpu_plain()).get_gpu_mem<uint64_t>()); 
        }

        inline Plaintext & gpu(bool val = true)
        {
            //on_gpu_ = true;
            ((xehe::plgin::XeHECiphertext &)get_gpu_plain()).on_gpu(val);
        ...
        {
            return (get_gpu_plain().is_on_gpu());
        }


        std::shared_ptr<xehe::ext::Buffer<uint64_t>>& lazy_upload(void)
        {
            auto upload_sz = dyn_array().size();
            if (get_gpu_mem() == nullptr)
            {
                get_gpu_mem() = xehe::ext::XeHE_malloc<uint64_t>(upload_sz);
            }

            if (is_dirty())
            {
                get_gpu_mem()->set_data(data(), upload_sz);
                set_dirty(false);
            }
...
#ifdef SEAL_USE_INTEL_XEHE
        // the plaintext is used as intermediate data for encryption and
        // sometimes as an operand
        // the oprand data has to be uploaded to keep t in sync with the host
        // the bit indicates the host update
        
        // For GPU memory migration purpose, we use XeHE Ciphertext class here inside SEAL plaintext class
        // XeHECipher text helps to manage dirty bit, gpu bit and prime64 bit of the underlying GPU Buffer.
        xehe::plgin::XeHECiphertext gpu_plain_;
#endif // SEAL_USE_INTEL_XEHE
