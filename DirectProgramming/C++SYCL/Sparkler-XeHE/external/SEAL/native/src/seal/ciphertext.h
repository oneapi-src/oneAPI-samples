    ...
    */
    ...
    public:
        ...
        */
        ...
        @throws std::invalid_argument if pool is uninitialized
        */
        Ciphertext(bool use_gpu, MemoryPoolHandle pool = MemoryManager::GetPool()) : data_(std::move(pool))
        {
            gpu(use_gpu);
        }
        ...
        @throws std::invalid_argument if pool is uninitialized
        */
        explicit Ciphertext(const SEALContext &context, bool use_gpu, MemoryPoolHandle pool = MemoryManager::GetPool())
            : data_(std::move(pool))
        {
             gpu(use_gpu);
            // Allocate memory but don't resize
        ...
        */
        explicit Ciphertext(
            const SEALContext &context, parms_id_type parms_id, bool use_gpu, MemoryPoolHandle pool = MemoryManager::GetPool())
            : data_(std::move(pool))
        {
            gpu(use_gpu);
            // Allocate memory but don't resize
        ...
        */
        explicit Ciphertext(
            const SEALContext &context, parms_id_type parms_id, bool use_gpu, std::size_t size_capacity,
            MemoryPoolHandle pool = MemoryManager::GetPool())
            : data_(std::move(pool))
        {
            gpu(use_gpu);
        ...
        */
        ...
        const xehe::plgin::XeHECiphertext &get_gpu_cipher(void) const
        {
            return (gpu_cipher_);
        }

        xehe::plgin::XeHECiphertext &get_gpu_cipher(void)
        {
            return (gpu_cipher_);
        }
        ...
        {
            ((xehe::plgin::XeHECiphertext &)get_gpu_cipher()).set_prime64(val);
        }

        inline bool is_prime64(void) const
        {
            return(get_gpu_cipher().is_prime64());
        }

        inline void set_dirty(bool val)
        {
            ((xehe::plgin::XeHECiphertext &)get_gpu_cipher()).set_dirty(val);
        } 

        inline bool is_dirty(void) const 
        {
             return (get_gpu_cipher().is_dirty());
        }

        const std::shared_ptr<xehe::ext::Buffer<uint64_t>>& get_gpu_mem(void) const
        {
            return (get_gpu_cipher().get_gpu_mem<uint64_t>()); //(gpu_mem_);
        }
        std::shared_ptr<xehe::ext::Buffer<uint64_t>>& get_gpu_mem(void)
        {
            return (((xehe::plgin::XeHECiphertext &)get_gpu_cipher()).get_gpu_mem<uint64_t>()); 
        }

        inline Ciphertext & gpu(bool val = true)
        {
            //on_gpu_ = true;
            ((xehe::plgin::XeHECiphertext &)get_gpu_cipher()).on_gpu(val);

            return (*this);
        }

        inline bool on_gpu(void) const
        {
            return (get_gpu_cipher().is_on_gpu());
        }
        ...
        friend void xehe_copy_ciphertext(Ciphertext &self, Ciphertext &assign);

#endif // SEAL_USE_INTEL_XEHE
    ...
    private:
        ...
        void resize_gpu_internal(size_t new_size, bool discard_alloc = false)
        {              
            ((xehe::plgin::XeHECiphertext &)get_gpu_cipher()).resize(new_size, discard_alloc); 
        }

        // gpu abstruction
        xehe::plgin::XeHECiphertext gpu_cipher_;
