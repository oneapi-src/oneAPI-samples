    ...
    */
    ...
    public:
        ...
        */
        ...
        void ntt_inplace(Ciphertext &encrypted);

        void invntt_inplace(Ciphertext &encrypted);

        void multiply_plain_add_inplace(
             Ciphertext &encrypted_add, const Ciphertext &encrypted_mul, const Plaintext &plain
             /*, MemoryPoolHandle pool = MemoryManager::GetPool()*/);
        ...
        destination = encrypted_mul * plain + encrypted_add
        @param[in] encrypted_mul The encrypted input for multiply
        @param[in] plain The plain input
        @param[in] encrypted_add The encrypted input for add
        @param[out] destination The output ciphertext
        */
        inline void multiply_plain_add(
             const Ciphertext &encrypted_add, const Ciphertext &encrypted_mul, const Plaintext &plain,Ciphertext &destination
            /*MemoryPoolHandle pool = MemoryManager::GetPool()*/)
        {
            destination = encrypted_add;
            multiply_plain_add_inplace(destination, encrypted_mul, plain/*, std::move(pool)*/);
        }
        ...
        destination = encrypted1 * encrypted2 + encrypted_add
        @param[in] encrypted1 The input1 for multiply
        @param[in] encrypted2 The input2 for multiply
        @param[in] encrypted_add The input for add
        @param[out] destination The output ciphertext
        */
        void multiply_add_inplace(
            Ciphertext &encrypted_add, const Ciphertext &encrypted1, const Ciphertext &encrypted2
            /*, MemoryPoolHandle pool = MemoryManager::GetPool()*/);

        inline void multiply_add(
            const Ciphertext &encrypted_add, const Ciphertext &encrypted1, const Ciphertext &encrypted2, Ciphertext &destination
            /*MemoryPoolHandle pool = MemoryManager::GetPool()*/)
        {
            destination = encrypted_add;
            multiply_add_inplace(destination, encrypted1, encrypted2/*, std::move(pool)*/);
        }
...
#ifdef SEAL_USE_INTEL_XEHE
        bool is_prime64(void) const
        {
            return (context_.is_prime64());
        }

        bool is_ckks(void);
        xehe::plgin::XEHEvaluator<uint64_t>& get_xehe_plugin(void);

        void* get_xehe_plugin_ptr(void);

        xehe::plgin::XEHEvaluator<uint64_t> *plgin64_ = nullptr;
        xehe::plgin::XEHEvaluator<uint32_t> *plgin32_ = nullptr;
