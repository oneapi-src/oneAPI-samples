...
#include "seal/xehe_seal_plgin.hpp"
#endif
...
namespace seal
{
    namespace
    {
        ...
        SEAL_NODISCARD inline bool are_same_scale_mad(const T &value1, const S &value2, const U &value3) noexcept
        {
            return util::are_close<double>(value1.scale() * value2.scale(), value3.scale());
        }
#endif
...
#ifdef SEAL_USE_INTEL_XEHE
    void Evaluator::ntt_inplace(Ciphertext &encrypted){
        auto plgin = get_xehe_plugin_ptr();
        if (is_prime64())
        {
            auto plgin64 = (xehe::plgin::XEHEvaluator<uint64_t> *)plgin;
            plgin64->ntt(encrypted);
        }
        else
        {
            auto plgin32 = (xehe::plgin::XEHEvaluator<uint32_t> *)plgin;
            plgin32->ntt(encrypted);
        }
        //get_xehe_plugin().ntt(encrypted);
    }

    void Evaluator::invntt_inplace(Ciphertext &encrypted){
        get_xehe_plugin().inverse_ntt(encrypted);
    }
#endif
...
#ifdef SEAL_USE_INTEL_XEHE
        if (is_ckks() && encrypted.on_gpu())
        {
            get_xehe_plugin().negate(encrypted, encrypted);
        }
        else
#endif
        {
...
#ifdef SEAL_USE_INTEL_XEHE
        if (is_ckks() && (encrypted1.on_gpu()|| encrypted2.on_gpu()))
        {
            get_xehe_plugin().add_inplace(encrypted1, encrypted2);
        }
        else
#endif
        {
    ...
    void Evaluator::sub_inplace(Ciphertext &encrypted1, const Ciphertext &encrypted2)
    {
        ...
        if (is_ckks() && (encrypted1.on_gpu() || encrypted2.on_gpu()))
        {
            get_xehe_plugin().sub_inplace(encrypted1, encrypted2);
        }
        else
#endif
        {
    ...
    void Evaluator::multiply_inplace(Ciphertext &encrypted1, const Ciphertext &encrypted2, MemoryPoolHandle pool)
    {
        ...
        case scheme_type::ckks:
            ...
            {
                get_xehe_plugin().multiply_inplace_ckks(encrypted1, encrypted2);
            }
            else
#endif
            {
    ...
    void Evaluator::square_inplace(Ciphertext &encrypted, MemoryPoolHandle pool)
    {
        ...
        case scheme_type::ckks:
            ...
            if (encrypted.on_gpu())
            {
                get_xehe_plugin().square_inplace_ckks(encrypted);
            }
            else
#endif
            {
        ...
        Ciphertext &encrypted, const RelinKeys &relin_keys, size_t destination_size, MemoryPoolHandle pool)
    {
        ...
        if (is_ckks() && encrypted.on_gpu())
        {
            get_xehe_plugin().relinearize_internal(encrypted, relin_keys, 2);
        }
        else
#endif
        {
    ...
    void Evaluator::mod_switch_to_next(const Ciphertext &encrypted, Ciphertext &destination, MemoryPoolHandle pool)
    {
        ...
        case scheme_type::ckks:
            ...
            {
                get_xehe_plugin().mod_switch_drop_to_next_ckks(encrypted, destination);
            }
            else
#endif
            {
    ...
    void Evaluator::rescale_to_next(const Ciphertext &encrypted, Ciphertext &destination, MemoryPoolHandle pool)
    {
        ...
        case scheme_type::ckks:
            ...
            if (encrypted.on_gpu() || destination.on_gpu())
            {
                get_xehe_plugin().mod_switch_scale_to_next(encrypted, destination, true);
            }
            else
#endif
            // Modulus switching with scaling
    ...
    void Evaluator::rescale_to_inplace(Ciphertext &encrypted, parms_id_type parms_id, MemoryPoolHandle pool)
    {
        ...
        case scheme_type::ckks:
            while (encrypted.parms_id() != parms_id)
            {
                ...
                if (encrypted.on_gpu())
                {
                    get_xehe_plugin().mod_switch_scale_to_next(encrypted, encrypted, true);
                }
                else
#endif
                // Modulus switching with scaling
    ...
    void Evaluator::add_plain_inplace(Ciphertext &encrypted, const Plaintext &plain)
    {
        ...
        case scheme_type::ckks:
        {
            ...
            if (encrypted.on_gpu())
            {
                get_xehe_plugin().add_plain_inplace_ckks(encrypted, plain);
            }
            else
#endif
            {
    ...
    void Evaluator::sub_plain_inplace(Ciphertext &encrypted, const Plaintext &plain)
    {
        ...
        case scheme_type::ckks:
        {
            ...
            if (encrypted.on_gpu())
            {
                get_xehe_plugin().sub_plain_inplace_ckks(encrypted, plain);
            }
            else
#endif
            {
    ...
    void Evaluator::multiply_plain_inplace(Ciphertext &encrypted, const Plaintext &plain, MemoryPoolHandle pool)
    {
        ...
        if (encrypted.is_ntt_form())
        {
            ...
            if (is_ckks() && encrypted.on_gpu())
            {
                get_xehe_plugin().mul_plain_inplace_ckks(encrypted, plain);
            }
            else
#endif
            {
        ...
        Ciphertext &encrypted, uint32_t galois_elt, const GaloisKeys &galois_keys, MemoryPoolHandle pool)
    {
        ...
        if (is_ckks() && encrypted.on_gpu())
        {
            get_xehe_plugin().apply_galois_inplace(encrypted, galois_elt, galois_keys);
        }
        else
#endif
        {
            ...
            switch_key_inplace(
                encrypted, temp, static_cast<const KSwitchKeys &>(galois_keys), GaloisKeys::get_index(galois_elt),
                pool);
        }
        ...
        Ciphertext &encrypted, int steps, const GaloisKeys &galois_keys, MemoryPoolHandle pool)
    {
        ...
        {
#ifdef SEAL_USE_INTEL_XEHE
            if (encrypted.on_gpu() && is_ckks())
            {
                get_xehe_plugin().apply_galois_inplace(encrypted, galois_tool->get_elt_from_step(steps), galois_keys);
            }
            else
#endif
            {
    ...
    void Evaluator::multiply_add_inplace(Ciphertext &encrypted_add, const Ciphertext &encrypted1, const Ciphertext &encrypted2/*, MemoryPoolHandle pool*/)
    {
        // Verify parameters.
        if (!is_metadata_valid_for(encrypted1, context_) || !is_buffer_valid(encrypted1))
        {
        ...
        if (!is_metadata_valid_for(encrypted_add, context_) || !is_buffer_valid(encrypted_add))
        {
            throw invalid_argument("encrypted_add is not valid for encryption parameters");
        }
        if (encrypted1.parms_id() != encrypted2.parms_id())
        {
            throw invalid_argument("encrypted1 and encrypted2 parameter mismatch");
        }
        if (encrypted1.parms_id() != encrypted_add.parms_id())
        {
            throw invalid_argument("encrypted1 and encrypted_add parameter mismatch");
        }
        if (!are_same_scale_mad(encrypted1, encrypted2, encrypted_add))
        {
            throw invalid_argument("scale mismatch");
        }

        auto context_data_ptr = context_.first_context_data();
        if (is_ckks() && encrypted_add.is_ntt_form() && (encrypted1.on_gpu() || encrypted2.on_gpu() || encrypted_add.on_gpu()))
        {
            get_xehe_plugin().multiply_add_inplace_ckks(encrypted_add, encrypted1, encrypted2);
        }
#ifdef SEAL_THROW_ON_TRANSPARENT_CIPHERTEXT
        if (!encrypted_add.on_gpu() && encrypted_add.is_transparent())
            throw logic_error("result ciphertext is transparent");
#endif
    }

    void Evaluator::multiply_plain_add_inplace(Ciphertext &encrypted_add, const Ciphertext &encrypted_mul, const Plaintext &plain/*, MemoryPoolHandle pool*/)
    {
        // Verify parameters.
        if (!is_metadata_valid_for(encrypted_mul, context_) || !is_buffer_valid(encrypted_mul))
        {
            throw invalid_argument("encrypted_mul is not valid for encryption parameters");
        }
        if (!is_metadata_valid_for(plain, context_) || !is_buffer_valid(plain))
        {
            throw invalid_argument("plain is not valid for encryption parameters");
        ...
        if (encrypted_mul.is_ntt_form() != plain.is_ntt_form())
        {
            throw invalid_argument("NTT form mismatch");
        }
        if (encrypted_add.is_ntt_form() != plain.is_ntt_form())
        {
            throw invalid_argument("NTT form mismatch");
        }
        if (!are_same_scale_mad(encrypted_mul, plain, encrypted_add))
        {
            throw invalid_argument("scale mismatch");
        }
        /*
        ...
        */
        if (encrypted_add.is_ntt_form() && is_ckks() && (encrypted_add.on_gpu() || encrypted_mul.on_gpu())){
            get_xehe_plugin().multiply_plain_add_inplace_ckks(encrypted_add, encrypted_mul, plain);
        }
    ...
    bool Evaluator::is_ckks(void) {
        auto& context_data = *(context_.key_context_data());
        bool ckks_scheme = (context_data.parms().scheme() == seal::scheme_type::ckks);
        return(ckks_scheme);
    }

    xehe::plgin::XEHEvaluator<uint64_t>& Evaluator::get_xehe_plugin(void) {
        {
            std::lock_guard<std::mutex> lk(init_lock_);
            if (plgin64_ == nullptr)
            {
                plgin64_ = new  xehe::plgin::XEHEvaluator<uint64_t>;
                plgin64_->init(&context_);
            }
        }
        return(*plgin64_);
    }
    ...
    void* Evaluator::get_xehe_plugin_ptr(void) {
        void *ret = nullptr;
        if (is_prime64())
        {
            {
        ...
        {
            {
                ...
                if (plgin32_ == nullptr)
                {
                    plgin32_ = new  xehe::plgin::XEHEvaluator<uint32_t>;
                    plgin32_->init(&context_);
                }
            }
            ret = plgin32_;
        }
