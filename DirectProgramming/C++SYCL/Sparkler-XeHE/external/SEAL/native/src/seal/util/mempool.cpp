              ...
              item_count_(MemoryPool::first_alloc_count), first_item_(nullptr)
        {
            ...
            try
            {
                ...
                new_alloc.data_ptr = static_cast<seal_byte *>(
                    std::aligned_alloc(64, mul_safe(MemoryPool::first_alloc_count, item_byte_count_)));
#else
        ...
        MemoryPoolItem *MemoryPoolHeadMT::get()
        {
            ...
            if (old_first == nullptr)
            {
                ...
                else
                {
                    ...
                    {
#ifdef SEAL_USE_ALIGN_64
                        new_alloc.data_ptr = static_cast<seal_byte *>(std::aligned_alloc(64, new_alloc_byte_count));
#else
...
#ifdef SEAL_USE_ALIGN_64
                new_alloc.data_ptr =
                    new (std::align_val_t{ 64 }) seal_byte[mul_safe(MemoryPool::first_alloc_count, item_byte_count_)];
#else
                new_alloc.data_ptr = new seal_byte[mul_safe(MemoryPool::first_alloc_count, item_byte_count_)];
#endif
            }
