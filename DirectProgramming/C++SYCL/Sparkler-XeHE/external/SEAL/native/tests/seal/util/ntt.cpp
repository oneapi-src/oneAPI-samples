...
namespace sealtest
{
    namespace util
    {
        ...
        TEST(NTTTablesTest, InverseNegacyclicNTTTest32)
        {
            MemoryPoolHandle pool = MemoryPoolHandle::Global();
            Pointer<NTTTables> tables;

            int coeff_count_power = 13;
            auto n = (size_t(1) << coeff_count_power);
            auto gen_modulus = seal::util::get_primes(n, 28, 1);

            //Modulus modulus(0xffffffffffc0001ULL);
            auto modulus = gen_modulus[0];
            std::cout << "Poly size " << n << " mod " << std::hex << modulus.value() << std::endl;
            ASSERT_NO_THROW(tables = allocate<NTTTables>(pool, coeff_count_power, modulus, pool));
            auto poly(allocate_zero_poly(n, 1, pool));
            auto temp(allocate_zero_poly(n, 1, pool));
        ...
        TEST(NTTTablesTest, InverseNegacyclicNTTTest64)
        {
            MemoryPoolHandle pool = MemoryPoolHandle::Global();
            Pointer<NTTTables> tables;
            ...
            int coeff_count_power = 13;
            auto n = (size_t(1) << coeff_count_power);
            auto gen_modulus = seal::util::get_primes(n, 60, 1);
