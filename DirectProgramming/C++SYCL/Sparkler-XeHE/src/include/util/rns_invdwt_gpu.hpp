...
#ifndef _RNS_INVDWT_GPU_HPP_
#define _RNS_INVDWT_GPU_HPP_
...
namespace xehe
{
    namespace util
    {
        ...
        */
        ...
        class RnsInvDwtGap {
        public:
            RnsInvDwtGap(int q_base_size, std::size_t log_n, std::size_t log_m, std::size_t log_gap, int rounds,
        ...
        template <typename T>
        class RnsInvDwtGapRadix2Fused {
        public:
            RnsInvDwtGapRadix2Fused(int q_base_size, std::size_t log_n, std::size_t log_m, std::size_t log_gap, int rounds,
                         T* values, const T* modulus,
                         const T* roots_op,
                         const T* roots_quo,
            ...
            void kernel(uint64_t poly_idx, uint64_t q, uint64_t ind) const {
                ...
                auto modulus = modulus_[q];
                auto two_times_modulus = (modulus << 1);
                auto scalar_op = scalar_op_[q];
                auto scalar_quo = scalar_quo_[q];
                std::size_t global_offset = (q << log_n_);
        ...
        template <typename T>
        class RnsInvDwtGapRadix4 {
        public:
            RnsInvDwtGapRadix4(int q_base_size, std::size_t log_n, std::size_t log_m, std::size_t log_gap, int rounds, T* values, const T* modulus,
        ...
        template <typename T>
        class RnsInvDwtGapRadix4Fused {
        public:
            RnsInvDwtGapRadix4Fused(int q_base_size, std::size_t log_n, std::size_t log_m, std::size_t log_gap, int rounds, T* values, const T* modulus,
        ...
        template <typename T>
        class RnsInvDwtGapRadix8 {
        public:
            RnsInvDwtGapRadix8(int q_base_size, std::size_t log_n, std::size_t log_m, std::size_t log_gap, int rounds, T* values, const T* modulus,
                      const T* roots_op, const T* roots_quo)
            {
                q_base_size_ = q_base_size;
                values_ = values;
        ...
        template <typename T>
        class RnsInvDwtGapRadix8Fused {
        public:
            RnsInvDwtGapRadix8Fused(int q_base_size, std::size_t log_n, std::size_t log_m, std::size_t log_gap, int rounds, T* values, const T* modulus,
                      const T* roots_op, const T* roots_quo, const T* scalar_op, const T* scalar_quo)
            {
                q_base_size_ = q_base_size;
                values_ = values;
        ...
        template <typename T>
        class RnsInvDwtLastRound {
        public:
            RnsInvDwtLastRound(int q_base_size, std::size_t log_n, std::size_t log_gap, int rounds, T* values, const T* modulus,
        ...
        template <typename T>
        class RnsInvDwtLastRoundScalar {
        public:
            RnsInvDwtLastRoundScalar(int q_base_size, std::size_t log_n, std::size_t log_gap, int rounds, T* values, const T* modulus,
                                     const T* roots_op, const T* roots_quo,
        ...
        template <typename T>
        class RnsInvDwtLastRoundScalarSeperate {
        public:
            RnsInvDwtLastRoundScalarSeperate(int q_base_size, std::size_t log_n, std::size_t log_gap, int rounds, T* values, const T* modulus,
                                     const T* roots_op, const T* roots_quo,
                                     const T* scalar_op, const T* scalar_quo, bool lazy = false)
            {
                q_base_size_ = q_base_size;
        ...
        class InvRnsDwt{
        public:
            InvRnsDwt(int poly_num, int q_base_size, int log_n, T* values, const T* modulus,
                      const T* roots_op, const T* roots_quo,
            ...
            void operator() [[intel::reqd_sub_group_size(SUB_GROUP_SIZE)]](void) {
                        ...
                        for (std::size_t i = 0; i < poly_num_; ++i) {
                            for (uint64_t q = 0; q < q_base_size_; ++q){
                                for (uint64_t j = 0; j < (1 << (log_m + log_gap)); ++j){
                                    RnsInvDwtGap<T, 0>(q_base_size_, log_n_, log_m, log_gap, total_r,
                                                                         values_, modulus_,
                        ...
                        for (std::size_t i = 0; i < poly_num_; ++i) {
                            for (uint64_t q = 0; q < q_base_size_; ++q){
                                for (uint64_t j = 0; j < (1 << (log_m + log_gap - log_unroll)); ++j){
                                    RnsInvDwtGap<T, log_unroll>(q_base_size_, log_n_, log_m, log_gap, total_r,
                                                                         values_, modulus_,
                                                                         roots_op_, roots_quo_).kernel(i, q, j);
                                }
                            }
                    ...
                    for (std::size_t i = 0; i < poly_num_; ++i) {
                        for (uint64_t q = 0; q < q_base_size_; ++q){
                            for (uint64_t j = 0; j < (1 << log_gap); ++j){
                                RnsInvDwtLastRoundScalar<T>(q_base_size_, log_n_, log_gap,
                                                                                          total_r, values_,
                ...
                else
                {
                    for (std::size_t i = 0; i < poly_num_; ++i) {
                        for (uint64_t q = 0; q < q_base_size_; ++q){
                            for (uint64_t j = 0; j < (1 << log_gap); ++j){
                                RnsInvDwtLastRound<T>(q_base_size_, log_n_, log_gap, total_r, values_,
                                                                        modulus_, roots_op_, roots_quo_, lazy_).kernel(i, q, j);
                            }
                        }
                    }
                }
...
#include "lib_utils.h"
        ...
        class RnsInvDwtGapLocalRadix4Fused {
        public:
            RnsInvDwtGapLocalRadix4Fused(int q_base_size, std::size_t log_n, std::size_t log_m, std::size_t log_gap, int rounds, T* values, const T* modulus,
        ...
        template <typename T, int unroll, int log_unroll>
        class RnsInvDwtGapLocalRadix4 {
        public:
            RnsInvDwtGapLocalRadix4(int q_base_size, std::size_t log_n, std::size_t log_m, std::size_t log_gap, int rounds, T* values, const T* modulus,
        ...
        template <typename T>
        class RnsInvDwtGapLocalRadix8Fused {
        public:
            RnsInvDwtGapLocalRadix8Fused(int q_base_size, std::size_t log_n, std::size_t log_m, std::size_t log_gap, int rounds, T* values, const T* modulus,
                      const T* roots_op, const T* roots_quo, const T* scalar_op, const T* scalar_quo, local_accessor<T, 1>& ptr) : ptr_{ptr}
            {
                q_base_size_ = q_base_size;
                values_ = values;
        ...
        template <typename T>
        class RnsInvDwtGapLocalRadix8 {
        public:
            RnsInvDwtGapLocalRadix8(int q_base_size, std::size_t log_n, std::size_t log_m, std::size_t log_gap, int rounds, T* values, const T* modulus,
                      const T* roots_op, const T* roots_quo, local_accessor<T, 1>& ptr) : ptr_{ptr}
            {
                q_base_size_ = q_base_size;
                values_ = values;
        ...
        template <typename T>
        class RnsInvDwtGapSIMD {
        public:
            RnsInvDwtGapSIMD(int q_base_size, std::size_t log_n, std::size_t log_m, std::size_t log_gap, int rounds, T* values, const T* modulus,
                      const T* roots_op, const T* roots_quo)
            {
                q_base_size_ = q_base_size;
                values_ = values;
        ...
        template <typename T, int log_Unroll>
        class RnsInvDwtGapLocal {
        public:
            RnsInvDwtGapLocal(int TER_LOCAL_MEM_GAP_SIZE, int q_base_size, std::size_t log_n, std::size_t log_m, std::size_t log_gap, int rounds, T* values, const T* modulus,
                      const T* roots_op, const T* roots_quo, local_accessor<T, 1>& ptr) : ptr_{ptr}
            {
        ...
        template <typename T, int log_Unroll>
        class RnsInvDwtGapLocalFused {
        public:
            RnsInvDwtGapLocalFused(int TER_LOCAL_MEM_GAP_SIZE, int q_base_size, std::size_t log_n, std::size_t log_m, std::size_t log_gap, int rounds, T* values, const T* modulus,
                      const T* roots_op, const T* roots_quo,
        ...
        template <typename T>
        class InvRnsDwtGpu : public InvRnsDwt<T> {
        public:
            InvRnsDwtGpu(int poly_num, int q_base_size, int log_n, T* values, const T* modulus,
                         const T* roots_op, const T* roots_quo,
            ...
            void operator() [[intel::reqd_sub_group_size(SUB_GROUP_SIZE)]] (cl::sycl::queue& queue) {
                ...
                if ( (log_m + log_gap <= log_local_unroll) || (log_m + log_gap <= LOG_LOCAL_REG_SLOTS) ){
                    ...
                    if ( (log_n_ % 3) == 0 ){// radix-8
                        ...
                        {
                            auto grid_range = sycl::range<3>(poly_num_, q_base_size_ ,(n>>3) );
                            auto e = queue.submit([&](cl::sycl::handler& h)
                            [[intel::reqd_sub_group_size(_SIMD_WIDTH_)]]
                            {
                                h.parallel_for({ grid_range }, RnsInvDwtGapRadix8<T>(q_base_size_, log_n_, log_m, log_gap, total_r, values_, modulus_,
                                                                                        roots_op_, roots_quo_));
                            });//.wait();
                            EventCollector::add_event("RnsInvDwtGapRadix8", e);
                        }
                        auto grid_range = sycl::range<3>(poly_num_, q_base_size_, (n >> 1));
                        auto e = queue.submit([&](cl::sycl::handler& h)
                        [[intel::reqd_sub_group_size(_SIMD_WIDTH_)]]
                        {
                            h.parallel_for({ grid_range }, RnsInvDwtLastRoundScalarSeperate<T>(q_base_size_, log_n_, log_n_ -1, total_r, values_, modulus_,
                                                                            roots_op_, roots_quo_, scalar_op_, scalar_quo_, lazy_));
                        });//.wait();
                        EventCollector::add_event("RnsInvDwtLastRoundScalarSeperate", e);
                        ...
                        for (; log_gap <= log_n_ - 1;total_r +=( m + (m>>1) ), m >>= 2, log_m -= 2, gap <<= 2, log_gap += 2)
                        {
                            ...
                            [[intel::reqd_sub_group_size(_SIMD_WIDTH_)]]
                            {
                                h.parallel_for({ grid_range }, RnsInvDwtGapRadix4<T>(q_base_size_, log_n_, log_m, log_gap, total_r, values_, modulus_,
                                                                                        roots_op_, roots_quo_));
                            });//.wait();
                            EventCollector::add_event("RnsInvDwtGapRadix4", e);
                        }
                        auto grid_range = sycl::range<3>(poly_num_, q_base_size_, (n >> 1));
                        auto e = queue.submit([&](cl::sycl::handler& h)
                        [[intel::reqd_sub_group_size(_SIMD_WIDTH_)]]
                        ...
                        for (; m > 0; total_r += m, m >>= 1, log_m--, gap <<= 1, log_gap++)
                        {
                            ...
                            auto grid_range = sycl::range<3>(poly_num_, q_base_size_, (1 << (log_m + log_gap - log_unroll)));
                            auto e = queue.submit([&](cl::sycl::handler& h)
                            [[intel::reqd_sub_group_size(_SIMD_WIDTH_)]]
                            {
                                h.parallel_for({ grid_range }, RnsInvDwtGap<T, log_unroll>(q_base_size_, log_n_, log_m, log_gap, total_r, values_, modulus_,
                                                                                            roots_op_, roots_quo_));
                            });//.wait();
                            EventCollector::add_event("RnsInvDwtGap", e);
                        }
                        auto grid_range = sycl::range<3>(poly_num_, q_base_size_, (n >> 1));
                        auto e = queue.submit([&](cl::sycl::handler& h)
                        [[intel::reqd_sub_group_size(_SIMD_WIDTH_)]]
                    ...
                    if (sizeof(T) == 4){
                        ...
                        if ( (MAX_WORK_GROUP_SIZE < 512) || (n >= 65536) || (n <= 2048 && n != 512) ) {// cannot be handled by radix-8 local-mem kernel, degrade to previous implementation
                            ...
                            {
                                h.parallel_for(nd_grid_range, RnsInvDwtGapSIMD<T>(q_base_size_, log_n_, log_m, log_gap, total_r, values_, modulus_,
                                                                                                                roots_op_, roots_quo_));
                            });//.wait();
                            EventCollector::add_event("RnsInvDwtGapSIMD", e1);
                            ...
                            if (terminate_at_local_mem && simd_not_overlap_with_local && last_round_in_local_mem && unroll_cover_last_round_in_local_kernel) {
...
#ifndef WIN32
                                    h.parallel_for(nd_grid_range_local, RnsInvDwtGapLocalFused<T, log_local_unroll>(TER_LOCAL_MEM_GAP_SIZE, q_base_size_, log_n_, log_m, log_gap, total_r, values_, modulus_,
                                                                                                            roots_op_, roots_quo_, scalar_op_, scalar_quo_, shared_buffer, lazy_));
#else
#if (5 < LOG_TER_GAP_SIZE)
                                    h.parallel_for(nd_grid_range_local, RnsInvDwtGapLocalFused<T, 5>(TER_LOCAL_MEM_GAP_SIZE, q_base_size_, log_n_, log_m, log_gap, total_r, values_, modulus_,
                                        roots_op_, roots_quo_, scalar_op_, scalar_quo_, shared_buffer, lazy_));
#else
                                    h.parallel_for(nd_grid_range_local, RnsInvDwtGapLocalFused<T, LOG_TER_GAP_SIZE>(TER_LOCAL_MEM_GAP_SIZE, q_base_size_, log_n_, log_m, log_gap, total_r, values_, modulus_,
                                        roots_op_, roots_quo_, scalar_op_, scalar_quo_, shared_buffer, lazy_));
#endif

#endif
                                });//.wait();
                                EventCollector::add_event("RnsInvDwtGapLocalFused", e2);
                            }else{
                                ...
                                auto e2 = queue.submit([&](cl::sycl::handler& h)
                                {
                                    auto shared_buffer = local_accessor<T, 1>(MAX_ELE_NUMBER_LOCAL_MEM, h);
    #ifndef WIN32
                                    h.parallel_for(nd_grid_range_local, RnsInvDwtGapLocal<T, log_local_unroll>(TER_LOCAL_MEM_GAP_SIZE, q_base_size_, log_n_, log_m, log_gap, total_r, values_, modulus_,
                                                                                                            roots_op_, roots_quo_, shared_buffer));
    #else
    #if (5 < LOG_TER_GAP_SIZE) 
                                    h.parallel_for(nd_grid_range_local, RnsInvDwtGapLocal<T, 5>(TER_LOCAL_MEM_GAP_SIZE, q_base_size_, log_n_, log_m, log_gap, total_r, values_, modulus_,
                                        roots_op_, roots_quo_, shared_buffer));
    #else
                                    h.parallel_for(nd_grid_range_local, RnsInvDwtGapLocal<T, LOG_TER_GAP_SIZE>(TER_LOCAL_MEM_GAP_SIZE, q_base_size_, log_n_, log_m, log_gap, total_r, values_, modulus_,
                                        roots_op_, roots_quo_, shared_buffer));
    #endif

    #endif
                                });//.wait();
                                EventCollector::add_event("RnsInvDwtGapLocal", e2);
                                }
                                ...
                                for (; m > 1; total_r += m, m >>= 1, log_m--, gap <<= 1, log_gap++)
                                {
                                    ...
                                    EventCollector::add_event("RnsInvDwtGap", e3);
                                }

                                if (scalar_op_ != nullptr && scalar_quo_ != nullptr)
                                {
                                    auto grid_range = sycl::range<3>(poly_num_, q_base_size_, (1 << log_gap));
                                    auto e3 = queue.submit([&](cl::sycl::handler& h)
                                    [[intel::reqd_sub_group_size(_SIMD_WIDTH_)]]
                                    {
                                        h.parallel_for({ grid_range }, RnsInvDwtLastRoundScalar<T>(q_base_size_, log_n_, log_gap, total_r, values_, modulus_,
                                                                                                roots_op_, roots_quo_, scalar_op_, scalar_quo_ , lazy_));
                                    });//.wait();
                                    EventCollector::add_event("RnsInvDwtLastRoundScalar", e3);
                                }
                                else
                                {
                                    auto grid_range = sycl::range<3>(poly_num_, q_base_size_, (1 << log_gap));
                                    ...
                                    [[intel::reqd_sub_group_size(_SIMD_WIDTH_)]]
                                    {
                                        h.parallel_for({ grid_range }, RnsInvDwtLastRound<T>(q_base_size_, log_n_, log_gap, total_r, values_, modulus_,
                                                                                            roots_op_, roots_quo_, lazy_));
                                    });//.wait();
                                    EventCollector::add_event("RnsInvDwtLastRound", e3);
                                }
                            }
                            ...
                            if (n == 4096 || n == 512){
                                ...
                                auto e = queue.submit([&](cl::sycl::handler& h)
                                {
                                    ...
                                    h.parallel_for(nd_grid_range_local, RnsInvDwtGapLocalRadix8Fused<T>(q_base_size_, log_n_, log_m, log_gap, total_r, values_, modulus_,
                                        roots_op_, roots_quo_, scalar_op_, scalar_quo_, shared_buffer));
                                });//.wait();
                                EventCollector::add_event("RnsInvDwtGapLocalRadix8Fused", e);
                            }else{
                                auto nd_grid_range_local = sycl::nd_range<3>({static_cast<uint32_t>(poly_num_),\
                                                                        static_cast<uint32_t>(q_base_size_) ,\
                                                                        static_cast<uint32_t>( (n >> 3) ) },\
                                ...
                                {
                                    auto shared_buffer = local_accessor<T, 1>(4096, h);
                                    h.parallel_for(nd_grid_range_local, RnsInvDwtGapLocalRadix8<T>(q_base_size_, log_n_, log_m, log_gap, total_r, values_, modulus_,
                                        roots_op_, roots_quo_, shared_buffer));
                                });//.wait();
                                EventCollector::add_event("RnsInvDwtGapLocalRadix8", e);
                                ...
                                if (n == 8192){
                                    ...
                                    [[intel::reqd_sub_group_size(_SIMD_WIDTH_)]]
                                    {
                                        h.parallel_for({ grid_range }, RnsInvDwtGapRadix2Fused<T>(q_base_size_, log_n_, log_m, log_gap, total_r, values_, modulus_,
                                                                                                    roots_op_, roots_quo_, scalar_op_, scalar_quo_));
                                    });//.wait();
                                    EventCollector::add_event("RnsInvDwtGapRadix2Fused", e1);
                                }else if (n == 16384){
                                    gap <<= 1; log_gap += 1;
                                    auto grid_range = sycl::range<3>(poly_num_, q_base_size_ , (n>>2) );
                                    auto e1 = queue.submit([&](cl::sycl::handler& h)
                                    [[intel::reqd_sub_group_size(_SIMD_WIDTH_)]]
                                    {
                                        h.parallel_for({ grid_range }, RnsInvDwtGapRadix4Fused<T>(q_base_size_, log_n_, log_m, log_gap, total_r, values_, modulus_,
                                                                                                roots_op_, roots_quo_, scalar_op_, scalar_quo_));
                                    });//.wait();
                                    EventCollector::add_event("RnsInvDwtGapRadix4Fused", e1); 
                                }else if (n == 32768){
                                    gap <<= 2; log_gap += 2;
                                    auto grid_range = sycl::range<3>(poly_num_, q_base_size_ , (n>>3) );
                                    auto e1 = queue.submit([&](cl::sycl::handler& h)
                                    [[intel::reqd_sub_group_size(_SIMD_WIDTH_)]]
                                    {
                                        h.parallel_for({ grid_range }, RnsInvDwtGapRadix8Fused<T>(q_base_size_, log_n_, log_m, log_gap, total_r, values_, modulus_,
                                                                                                roots_op_, roots_quo_, scalar_op_, scalar_quo_));
                                    });//.wait();
                                    EventCollector::add_event("RnsInvDwtGapRadix8Fused", e1); 
                                }
                        ...
                        if ( (MAX_WORK_GROUP_SIZE < 512) || (n >= 65536) || (n <= 2048) ) {// cannot be handled by radix-8 local-mem kernel, degrade to previous implementation
                            ...
                            if (n == 4096){
                                ...
                                {
                                    auto shared_buffer = local_accessor<T, 1>(4096, h);
                                    h.parallel_for(nd_grid_range_local, RnsInvDwtGapLocalRadix4Fused<T, 2, 1>(q_base_size_, log_n_, log_m, log_gap, total_r, values_, modulus_,
                                        roots_op_, roots_quo_, scalar_op_, scalar_quo_, shared_buffer));
                                });//.wait();
                                EventCollector::add_event("RnsInvDwtGapLocalRadix4Fused", e);
                                ...
                                {
                                    auto shared_buffer = local_accessor<T, 1>(4096, h);
                                    h.parallel_for(nd_grid_range_local, RnsInvDwtGapLocalRadix4<T, 2, 1>(q_base_size_, log_n_, log_m, log_gap, total_r, values_, modulus_,
                                        roots_op_, roots_quo_, shared_buffer));
                                });//.wait();
                                EventCollector::add_event("RnsInvDwtGapLocalRadix4", e);
                                ...
                                if (n == 8192){
                                    ...
                                    EventCollector::add_event("RnsInvDwtGapRadix4", e1);
                                    total_r += m + (m>>1); m >>= 2; log_m -= 2;gap <<= 1; log_gap += 1;

                                    auto grid_range1 = sycl::range<3>(poly_num_, q_base_size_ , (n>>1) );
                                    auto e2 = queue.submit([&](cl::sycl::handler& h)
                                    [[intel::reqd_sub_group_size(_SIMD_WIDTH_)]]
                                    {
                                        h.parallel_for({ grid_range1 }, RnsInvDwtGapRadix2Fused<T>(q_base_size_, log_n_, log_m, log_gap, total_r, values_, modulus_,
                                                                                                    roots_op_, roots_quo_, scalar_op_, scalar_quo_));
                                    });//.wait();
                                    EventCollector::add_event("RnsInvDwtGapRadix2Fused", e2);
                                }
                            }
...
#endif //#ifndef _RNS_INVDWT_GPU_HPP_
