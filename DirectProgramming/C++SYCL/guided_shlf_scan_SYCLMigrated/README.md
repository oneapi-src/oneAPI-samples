...
Reductions are a very common computation in parallel algorithms.  Any time an array of values needs to be reduced to a single value using a binary associative operator, a reduction can be used. This `guided_shfl_scal_migrated` sample implements a reduction kernel that reduces an arbitrary size array. The original CUDA* source code is migrated to SYCL for portability across GPUs from multiple vendors.
...
The guided_shfl_scal_migrated sample shows how to perform a reduction operation on an array of values using the sycl::atomic_fence function to produce a single value in a single kernel
...
This sample is migrated from NVIDIA CUDA sample. See the [shfl_scan](https://github.com/NVIDIA/cuda-samples/tree/e8568c417356f7e66bb9b7130d6be7e55324a519/Samples/2_Concepts_and_Techniques/shfl_scan) sample in the NVIDIA/cuda-samples GitHub.
...
2. cd cuda-samples/Samples/2_Concepts_and_Techniques/shfl_scan/
