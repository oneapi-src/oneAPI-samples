...
values never occurs in the input array, so one bin will have always have 0
value.
...
[Run the Histogram sample on the Intel&reg; DevCloud](#run-histogram-on-devcloud)
...
6.	Remove the stdout and stderr files and clean-up the project files.
    ...
    ```
7.	Disconnect from the Intel&reg; DevCloud.
    ```
    exit
    ```
