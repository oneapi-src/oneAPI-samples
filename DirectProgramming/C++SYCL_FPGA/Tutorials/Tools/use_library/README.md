...
1. `fpga_crossgen` creates object file that contains representations for target devices (FPGA) and FPGA emulator. The following command instructs `fpga_crossgen` to generate SYCL target objects from an RTL source in this tutorial:
   ...
   > **Note**: When you are using special datatype (such as ac_intin this sample) in emulation model, you may safely ignore the warning complain about "incomplete type which could be incompatible with C". This warning would not affect the functionality of the design,
