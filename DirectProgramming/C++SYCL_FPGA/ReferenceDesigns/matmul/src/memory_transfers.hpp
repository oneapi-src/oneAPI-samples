...
template <typename TT,                // Datatype of the elements of the matrix
          ...
          int k_elems_per_ddr_access, // Number of elements per DDR burst access
...
void MatrixReadFromDDRToPipeA(
  ...
  constexpr int kNumBanks =
      fpga_tools::Pow2(fpga_tools::CeilLog2(k_rows_a / k_elems_per_ddr_access));

#if defined(IS_BSP)
  // When targeting a BSP, we instruct the compiler that this pointer lives on
  // the device.
  // Knowing this, the compiler won't generate hardware to potentially get data
  // from the host.
        ...
        fpga_tools::UnrolledLoop<kBlocksA>([&](auto k) {
          fpga_tools::UnrolledLoop<k_tile_a>([&](auto t) {
            pipe_write.template get<t>() =
                get[k]
                    ? mem[mat][i % k_common][k * k_tile_a + t]
...
void MatrixReadFromDDRToPipeB(
  ...
  constexpr short kBankWidth = k_elems_per_ddr_access * sizeof(TT);
  constexpr int kNumBanks =
      fpga_tools::Pow2(fpga_tools::CeilLog2(k_cols_b / k_elems_per_ddr_access));
      ...
      for (ac_int<kItersToPipeBitSize, false> i = 0; i < kItersToPipe; i++) {
        ...
        fpga_tools::UnrolledLoop<kBlocksB>([&](auto k) {
          fpga_tools::UnrolledLoop<k_tile_b>([&](auto t) {
            pipe_write.template get<t>() =
                get[k]
                    ? mem[mat][i % k_common][k * k_tile_b + t]
                    : sycl::ext::intel::fpga_reg(pipe_write.template get<t>());
          });
        });
...
void MatrixReadPipeToDDR(
  ...
  // Read every tile of all "k_num_matrices" matrices from the pipe into on-chip
