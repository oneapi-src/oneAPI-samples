...
## Purpose
This FPGA reference design demonstrates a simple Direct Memory Access (DMA) module produced with the Intel® oneAPI DPC++/C++ Compiler, and how to use a NIOSV soft CPU to interact with it.
...
The NIOS V soft CPU is connected to the other IPs in this design using Platform Designer. This design is stored in the file `pd_system.qsys`.

![](assets/platform-designer_top.png)

There are three important IPs in this system:

1. **On-chip Memory** This on-chip RAM is shared between the Nios V/g soft CPU and the simple DMA accelerator FPGA IP produced with the Intel® oneAPI DPC++/C++ Compiler.

2. **Nios V/g soft CPU** This soft CPU executes a simple test program to exercise the simple DMA accelerator FPGA IP produced with the Intel® oneAPI DPC++/C++ Compiler.
...
3. Instruct the DMA kernel to copy the data from the source buffer to the destination buffer
