...
using CsrInProperties = decltype(sycl::ext::oneapi::experimental::properties(
    sycl::ext::intel::experimental::protocol<
        // `ready` signal is required for input CSR pipe. Host may check it to
        // ensure the kernel is ready for a new value.
        sycl::ext::intel::experimental::protocol_name::avalon_mm_uses_ready>,
    // `valid` signal ensures that the first read after the host changes it will
    // succeed.
    sycl::ext::intel::experimental::uses_valid_on));
using CsrOutProperties = decltype(sycl::ext::oneapi::experimental::properties(
    sycl::ext::intel::experimental::protocol<
        // Disable `ready` signal so the device can update this value without
        // requiring the host to consent. Host doesn't care about possibly
        // missing an update.
...
class ID_BypassCSR;
using BypassCSR =
    sycl::ext::intel::experimental::pipe<ID_BypassCSR, bool, 0, CsrInProperties>;

class ID_VersionCSR;
using VersionCSR =
    sycl::ext::intel::experimental::pipe<ID_VersionCSR, int, 0, CsrOutProperties>;
