...
struct QFP {
  ...
  static qfp_type FromFP32(float f) {
    ...
    int fp32_exponent = f_casted.parts.exponent;
    int fp32_mantissa = f_casted.parts.mantissa;
  ...
  // HOST CODE ONLY
  //
  static constexpr float ToFP32(qfp_type i) {
    int sign_bit = 0;
    if constexpr (!is_signed) {
    ...
    for (int i = 1; i <= 23; i++) {
      mantissa_sum += ((fp32_mantissa >> (23-i)) & 0x1) * Pow(2, -i);
    }
