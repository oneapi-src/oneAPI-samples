...
struct QFP {
  ...
  static qfp_type FromFP32(float f) {
    ...
    ac_int<fp32_exponent_bits + 1, true> fp32_exponent =
        f_casted.parts.exponent;
    ac_int<fp32_mantissa_bits, false> fp32_mantissa = f_casted.parts.mantissa;
    ...
    auto qfp_exponent =
        fp32_exponent - fp32_exponent_offset + qfp_exponent_offset;
  ...
  static constexpr qfp_type FromFP32CE(float f) {
    ...
    int fp32_exponent = FP32ExtractExponent(f) + fp32_exponent_offset;
  ...
  static constexpr float ToFP32CE(qfp_type i) {
    ...
    for (int i = 1; i <= fp32_mantissa_bits; i++) {
      mantissa_sum +=
          ((fp32_mantissa >> (fp32_mantissa_bits-i)) & 0x1) * Pow(2, -i);
    }
