      ...
      while (1) {
        ...
        for (ac_int<kLoadIterBitSize, false> li = 0; li < kLoadIter; 
          ...
          UnrolledLoop<kNumBanks>([&](auto k) {
            UnrolledLoop<kNumElementsPerBank>([&](auto t) {
              ...
              pipeData = sycl::INTEL::fpga_reg(pipeData);
            });

            write_row_group = sycl::INTEL::fpga_reg(write_row_group);
          });
        }
        ...
        */
        ...
        for (int s = 0; s < kIterations; s++) {
          ...
          UnrolledLoop<kNumBanks>([&](auto k) {
            i_gt_0[k] = sycl::INTEL::fpga_reg(i > 0);
            i_lt_0[k] = sycl::INTEL::fpga_reg(i < 0);
            j_eq_i[k] = sycl::INTEL::fpga_reg(j == i);
            i_ge_0_j_ge_i[k] = sycl::INTEL::fpga_reg(i >= 0 && j >= i);
            j_eq_i_plus_1[k] = sycl::INTEL::fpga_reg(j == i + 1);
            int idx = j + kIncreasedBufferSize;
            sori[k] = sycl::INTEL::fpga_reg(s_or_i[idx]);
          });
        ...
        */
        ...
        for (ac_int<kStoreIterBitSize, false> si = 0; si < kStoreIter; 
          ...
          UnrolledLoop<kNumBanks>([&](auto k) {
            get[k] = desired == k;
            desired = sycl::INTEL::fpga_reg(desired);
          });
            ...
            UnrolledLoop<kNumElementsPerBank>([&](auto k) {
              ...
              if constexpr(rowIdx < rows){
                pipeData.template get<k>() = get[t] ? 
                              ...
                              sycl::INTEL::fpga_reg(pipeData.template get<k>());
              }
            });
          });

          QMatrixOutPipe::write(pipeData);
