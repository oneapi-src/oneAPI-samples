  ...
  for (size_t r = 0; r < reps; r++) {
    ...
    for (size_t bufferIdx = 0, it = 0; it < matrices;
      ...
      q.single_task<class QRD>(
          StreamingQRD<T, isComplex, rows, columns, rawLatency, matricesPerIter,
                       kNumElementsPerDDRBurst, AMatrixPipe, QMatrixPipe,
                       RMatrixPipe>());
      ...
      auto QEvent = q.submit([&](sycl::handler &h) {
        h.single_task<class QRD_local_mem_to_DDR_Q>([=
          ...
          MatrixReadPipeToDDR<TT, rows, columns, kNumElementsPerDDRBurst,
                              matricesPerIter, QMatrixPipe>(currentQBuffer);
        });
      });
      ...
      auto REvent = q.submit([&](sycl::handler &h) {
        h.single_task<class QRD_local_mem_to_DDR_R>([=
          ...
          VectorReadPipeToDDR<TT, kRMatrixSize, kNumElementsPerDDRBurst,
                              matricesPerIter, RMatrixPipe>(currentRBuffer);
        });
      });
