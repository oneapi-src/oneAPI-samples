...
void CholeskyDecompositionImpl(
  ...
  q.single_task<CholeskyLocalMemToDDRL>([=
    ...
    // one vector
    constexpr bool kIncompleteBurst = kLMatrixSize%kNumElementsPerDDRBurst != 0;
    constexpr int kExtraIteration = kIncompleteBurst ? 1 : 0;
    constexpr int kLoopIter = (kLMatrixSize / kNumElementsPerDDRBurst)
                              + kExtraIteration;
            ...
            for (int k = 0; k < kNumElementsPerDDRBurst; k++) {
              if(li * kNumElementsPerDDRBurst + k < kLMatrixSize){
                vector_ptr_device[matrix_index * kLMatrixSize
                          + li * kNumElementsPerDDRBurst + k] = l_result[li][k];
              }
            }
          }
          else{
