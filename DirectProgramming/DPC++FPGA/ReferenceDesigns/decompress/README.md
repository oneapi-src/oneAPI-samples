...
3. (Optional) As the above hardware compile may take several hours to complete, FPGA precompiled binaries (compatible with Linux* Ubuntu* 18.04) can be downloaded <a href="https://iotdk.intel.com/fpga-precompiled-binaries/latest/decompress.fpga.tar.gz" download>here</a>.
...
The following source files can be found in the `src/` sub-directory. The `src/common/` sub-directory contains datatypes, functions, and kernels that are common between the GZIP and SNAPPY decompression implementations. The `src/gzip/` and `src/snappy/` sub-directories contain kernels and functions that are specific to the GZIP and SNAPPY decompression implementations, respectively.
...
<img src="snappy_decompressor.png" alt="gzip_decompression_engine_full" width="700"/>
...
### Snappy
Snappy is compression format that aims for high throughput compression and decompression, at the expense of compression ratio. It is typically used to compress database files that are stored in column-oriented format, since, unlike rows, columns are likely to have similar values.
...
#### Preamble Format
The first bytes of the stream, often called the *preamble*, store the file's uncompressed length as a *varint*, which is in the range [0, 2^32). A *varint* consist of a series of bytes, where the lower 7 bits are data and the upper bit is set if there are more bytes to be read. For example, an uncompressed length of 64 would be stored as 0x40, and an uncompressed length of 2097150 (0x1FFFFE) would be stored as {0xFE, 0xFF, 0x7F}. Thus, the uncompressed length is stored in the first bytes of the stream, using a maximum of 5 bytes.
...
Each element starts with a tag byte, as illustrated in the figure below.

<img src="tag_format.png" alt="gzip_file_format" width="600"/>
...
##### Literal Strings
For literal strings up to and including 60 elements, the upper 6 bits in the tag contain the length - 1. For longer literals, the 6 extra tag bits indicate how many more bytes to read to get the length; 60, 61, 62 or 63 for 1, 2, 3, and 4 bytes, respectively. After decoding the length, the literal string itself follows. These two formats are illustrated int the figure below, respectively.

<img src="literal_tag_format.png" alt="gzip_file_format" width="600"/>

##### Copies
This design supports copies with offsets up to 65kB. Currently, the Snappy format does not produce offsets greater than 32kB. If the distance is increased beyond 65kB in the future, then this design will be updated accordingly.

As discussed earlier, there are 3 options for copies depending on the value of the : a 1-, 2-, or 4-byte distance. These three formats are illustrated in the figure below.

<img src="copy_tag_format.png" alt="gzip_file_format" width="600"/>
...
**4-byte copies** are the similar to 2-byte copies but the distance is stored in the 4-bytes following the tag byte and therefore can have offsets in the range [0, 4294967295]. As mentioned earlier, this copy type is not supported in the design because currently the Snappy format works on 32kB blocks and therefore will never generate offsets greater than 32kB.

### Snappy Decompression FPGA Design
The image below summarizes the full streaming Snappy decompression design. The orange kernels on the FPGA in the dashed box make up the streaming Snappy decompression engine. The `Producer` and `Consumer` kernels are only used to stream data into and out of the decompression engine from and to device memory, respectively.

<img src="snappy_decompressor.png" alt="gzip_decompression_engine_full" width="700"/>

The `SnappyReader` kernel reads the input stream and decodes it to stream out either literal strings or {length, distance} pairs to the `LZ77Decoder`. It can decode a tag byte and the subsequent extra bytes (at most 4 extra) in a single cycle. Thus, it is capable of providing the downstream `LZ77Decoder` with either a literal or a {length, distance} pair every cycle.

The `literals_per_cycle` parameter can be set by the user at compile-time, which controls both how many literals the `SnappyReader` can read from a literal string per cycle, as well as the number of literals the `LZ77Decoder` can read from the history buffer per cycle. The default value for this variable is 8 (see *main.cpp*) but it can be set at compile time using the `-DLITERALS_PER_CYCLE=<value>` flag.

The details for the `ByteStacker` and `LZ77Decoder` kernels are given in the earlier GZIP and DEFLATE section.