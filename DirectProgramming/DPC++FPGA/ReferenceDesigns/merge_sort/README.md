...
To achieve SIMD level parallelism, we enhance the merge unit to merge `k` elements per cycle. The figure below shows how this is done. We will assume that we are sorting from largest to smallest, but the logic is very similar for sorting largest to smallest and is easily configurable at compile time in this design. <br/>
