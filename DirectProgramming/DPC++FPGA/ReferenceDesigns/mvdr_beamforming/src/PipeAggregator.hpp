...
template <typename PipeArrayType,  // 1D pipe_array class
          ...
          size_t k_elements_per_array_pipe,  // number of T elements
                                             // read/written to each sub-pipe in
                                             // PipeArrayType
...
struct PipeAggregator {
  ...
  static void write(const LargePipeElement &data, bool &success) {
    ...
    UnrolledLoop<k_num_pipes>(
        [&](auto i) { large_pipe_success &= small_pipe_success[i]; });
  ...
  static LargePipeElement read(bool &success) {
    ...
    UnrolledLoop<k_num_pipes>([&](auto i) {
      SmallPipeElement pipe_data;
      pipe_data =
          PipeArrayType::template PipeAt<i>::read(small_pipe_success[i]);
      UnrolledLoop<k_elements_per_array_pipe>([&](auto j) {
