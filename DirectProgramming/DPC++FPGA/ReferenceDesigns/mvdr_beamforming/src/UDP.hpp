...
void SetupPAC(unsigned long fpga_mac_adr, char *fpga_ip_adr,
  ...
  //
  if ((res = fpgaWriteMMIO64(afc_handle, 0, CSR_FPGA_MAC_ADR, fpga_mac_adr)) != FPGA_OK) {
    fprintf(stderr, "Error: writing FPGA MAC CSR\n");
    std::terminate();
  }
  unsigned long tmp1 = htonl(inet_addr(fpga_ip_adr));
  if ((res = fpgaWriteMMIO64(afc_handle, 0, CSR_FPGA_IP_ADR, tmp1)) != FPGA_OK) {
    fprintf(stderr, "Error: writing FPGA IP CSR\n");
    std::terminate();
  }
  if ((res = fpgaWriteMMIO64(afc_handle, 0, CSR_FPGA_UDP_PORT, (unsigned long) fpga_udp_port)) != FPGA_OK) {
    fprintf(stderr, "Error: writing FPGA UDP port CSR\n");
    std::terminate();
  }
  unsigned long tmp2 = htonl(inet_addr(fpga_netmask));
  if ((res = fpgaWriteMMIO64(afc_handle, 0, CSR_FPGA_NETMASK, tmp2)) != FPGA_OK) {
    fprintf(stderr, "Error: writing FPGA netmask CSR\n");
    std::terminate();
  }
  if ((res = fpgaWriteMMIO64(afc_handle, 0, CSR_HOST_MAC_ADR, host_mac_adr)) != FPGA_OK) {
    fprintf(stderr, "Error: writing HOST MAC CSR\n");
    std::terminate();
  }
  unsigned long tmp3 = htonl(inet_addr(host_ip_adr));
  if ((res = fpgaWriteMMIO64(afc_handle, 0, CSR_HOST_IP_ADR, tmp3)) != FPGA_OK) {
    fprintf(stderr, "Error: writing HOST IP CSR\n");
    std::terminate();
  }
  if ((res = fpgaWriteMMIO64(afc_handle, 0, CSR_HOST_UDP_PORT, (unsigned long) host_udp_port)) != FPGA_OK) {
    fprintf(stderr, "Error:writing HOST UDP port CSR\n");
    std::terminate();
  }
  if ((res = fpgaWriteMMIO64(afc_handle, 0, CSR_PAYLOAD_PER_PACKET, (unsigned long) kUDPDataSize)) != FPGA_OK) {
    fprintf(stderr, "Error: writing payload per packet CSR\n");
    std::terminate();
  }
  if ((res = fpgaWriteMMIO64(afc_handle, 0, CSR_CHECKSUM_IP, (unsigned long) CHECKSUM_IP)) != FPGA_OK) {
  ...
  if ((res = fpgaReadMMIO64(afc_handle, 0, CSR_STATUS_REG, &read_tmp)) != FPGA_OK) {
...
void UDPSender(char *fpga_ip, unsigned int port, unsigned char *input_data,
               size_t packets,
               high_resolution_clock::time_point *t_in,
  ...
  for (int i = 0; i < packets; i++) {
    sendto(sock,
           input_data + i*kUDPTotalSize,
           kUDPTotalSize,
           0,
           (struct sockaddr *) &fpgaaddr,
           sizeof(fpgaaddr));
...
void UDPReceiver(char *host_ip, unsigned int port, unsigned char *output_data,
                 size_t packets,
                 high_resolution_clock::time_point *t_out) {
  int sock, res;
  struct ifreq ifr;
  ...
  if ((res = setsockopt(sock, SOL_SOCKET, SO_BINDTODEVICE, (void *)&ifr, sizeof(ifr))) < 0) {
...
unsigned char * AllocatePackets(size_t packets) {
  ...
  auto ret =
    static_cast<unsigned char*>(aligned_alloc(1024, kUDPTotalSize * packets));
...
void ToPackets(unsigned char *udp_bytes, T *data, size_t count) {
  ...
  for (int i = 0; i < iterations; i++) {
    ...
    memcpy(&udp_bytes[i * packet_stride + 2],
           &data[i * count_per_packet],
           kUDPDataSize);
  }
}
...
void FromPackets(unsigned char *udp_bytes, T *data, size_t count) {
  ...
  size_t packet_stride = kUDPDataSize + 2;
  for (int i = 0; i < iterations; i++) {
    memcpy(&data[i * count_per_packet],
           &udp_bytes[i * packet_stride + 2],
           kUDPDataSize);
  }
