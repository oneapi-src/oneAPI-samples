...
template <
  typename  ForwardSubstitutionKernelName,   // Name to use for the Kernel
  ...
  typename  LDiagRecipVectorInPipe,   // Receive 1/ the diagonal elements of L.
                                      // Receive one float per read.
...
event SubmitForwardSubstitutionKernel( queue& q ) {
  ...
  static_assert( k_vector_size > 0,
    "k_vector_size must be greater than 0" );
  static_assert(std::numeric_limits<short>::max() > k_vector_size,
    "k_vector_size must fit in a short" );
  static_assert( k_num_y_vectors > 0,
    "k_num_y_vectors must be greater than 0" );
  ...
  auto e = q.submit([&](handler& h) {
    ...
    h.single_task< ForwardSubstitutionKernelName > ( [=] {
      ...
      while( 1 ) {
        ...
        constexpr int kLoadLoopIterations = (kNumLElements > kNumYElements) ?
          kNumLElements : kNumYElements;
        ...
        for( int iteration = 0; iteration < kLoadLoopIterations;
             iteration++ ) {
