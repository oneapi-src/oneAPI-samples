...
template <
  typename  BeamformerKernelName,     // Name to use for the Kernel
...
event SubmitBeamformerKernel( 
  ...
  static_assert( k_num_elements >= 0,
    "k_num_elements must be greater than 0" );
  ...
  static_assert( k_num_elements % k_num_complex_per_xrx_read == 0, 
  ...
  static_assert( k_unroll_factor % k_num_complex_per_xrx_read == 0,
  ...
  auto e = q.submit([&](handler& h) {
    ...
    h.single_task< BeamformerKernelName > ( [=] {
      ...
      while( 1 ) {
        ...
        for( unsigned char vector_num = 0; vector_num < (char)k_num_weight_vectors;
             vector_num++ ) {
        ...
        for( int xrx_vector_num = 0; xrx_vector_num < num_xrx_per_weights;
          ...
          for(  short i = 0; i < kNumCalcTypePerVector * kReadsPerCalcType; 
            ...
            UnrolledLoop<k_num_complex_per_xrx_read>([&](auto k) {
              short subindex =  (i % kReadsPerCalcType) * 
                                (short)k_num_complex_per_xrx_read + k;
          ...
          for (unsigned char vector_num = 0;
               vector_num < (unsigned char)k_num_weight_vectors;
               vector_num++ ) {
            // zero the accumulators
            UnrolledLoop<k_unroll_factor>([&](auto i) {
              accum_vector[i] = 0;
            });
