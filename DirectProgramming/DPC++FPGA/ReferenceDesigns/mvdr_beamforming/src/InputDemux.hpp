...
template <
  typename  InputDemuxKernelName,     // Name to use for the Kernel
        ...
        if (!cur_all_training_sent || !cur_all_xrx_sent) {
          ...
          if( ready_to_send[cur_tx_buffer] && !cur_all_xrx_sent ) {
            XrxDataOutPipe::write( 
              xrx_data_matrix[cur_tx_buffer][cur_tx_xrx_count], write_success );
          } else {
            write_success = false;
          }
          if( write_success ) {
            all_xrx_sent =
              (cur_tx_xrx_count == reads_per_xrx_matrix-1);
            tx_xrx_count++;
          }
