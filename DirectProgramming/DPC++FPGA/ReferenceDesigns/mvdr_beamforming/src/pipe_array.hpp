...
template <
  class     Id,         // identifier for the pipe array
...
struct PipeArray {
  ...
  static constexpr size_t GetNumDims() {
    return( sizeof...(dims));
  }
  ...
  template <template <std::size_t...> class WriteFunc, typename... IndexSequences>
  static void write_currying_helper(
    const BaseTy &data, 
    bool &success, 
    IndexSequences...
  ) {
