...
template <
  typename  StreamingQRDKernelName,   // Name to use for the Kernel

  size_t k_min_inner_loop_iterations, // Minimum number of inner loop iterations
                                      // to achieve an outer loop II of 1.  This
                                      // value will have to be tuned for optimal
                                      // performance.  Refer to the Triangular
                                      // Loop design pattern tutorial.
  ...
  typename  RMatrixOutPipe,           // R output pipe.  Send one complex number
                                      ...
                                      // starting with row 0.  
  typename  RDiagRecipVectorOutPipe   // 1 / the value of each diagonal entry of
                                      // the R matrix.  Diagonals of R are real
                                      // valued, so only send one float per
                                      // write
>
    ...
    h.single_task< StreamingQRDKernelName > ( [=] {
      ...
      constexpr short kMinIterationsMinusCols = 
        (k_min_inner_loop_iterations > k_a_num_cols) ? 
          ((short)k_min_inner_loop_iterations - (short)k_a_num_cols) : 0;
      ...
      while( 1 ) {
        ...
        for(  short i = 0; 
              i < (short)k_a_num_rows*(short)k_a_num_cols / (short)k_pipe_width;
              i++ ) {
        ...
        for (int s = 0; s < kNumIterations; s++) {
          ...
          UnrolledLoop<k_a_num_rows>([&](auto row) {
            ...
            ComplexType sori_or_0 = i_lt_0[row / kNumElementsPerBank] ?
              0 : sori[row / kNumElementsPerBank];
            ...
            ComplexType vector_t_or_0 = j_eq_i[row / kNumElementsPerBank] ?
              0 : vector_t.template get<row>();
            vector_t.template get<row>() =  vector_ai_times_conj_sori_or_0 + 
                                            vector_t_or_0;
