...
/*
  ...
  automatically at compile time if the size is not a multiple of 
  num_elem_per_bank.
*/
template <typename TT,           // Datatype of the elements of the matrix
...
void VectorReadPipeToDDR(
    TT* vector_ptr,  // Output vector buffer
    typename std::enable_if_t<(size % num_elem_per_bank) == 0>* = 0) {
...
/*
  ...
  automatically at compile time if the size is a multiple of num_elem_per_bank.
*/
template <typename TT,         // Datatype of the elements of the matrix
          int size,            // Number of elements in the vector
          int num_elem_per_bank,  // Number of TT elements per DDR burst access
...
#pragma unroll
    for (int k = 0; k < num_elem_per_bank; k++) {
      if ((vector_idx + k) < size) {
        *(vector_ptr_device + vector_count_idx * size + vector_idx + k) =
            pipe_read.elem[k];
      }
    }

    // Update the indexes
    int vector_idx_plus_num_elem_per_bank = vector_idx + num_elem_per_bank;
    if (vector_idx_plus_num_elem_per_bank > size) {
      vector_idx = 0;
      vector_count_idx += 1;
    } else {
      vector_idx = vector_idx_plus_num_elem_per_bank;
    }
  }  // end of li
}
