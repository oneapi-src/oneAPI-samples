...
the `-Xsno-interleaving` flag: `-Xsno-interleaving=<global_memory_type>`. The
memory type is usually indicated in the board specification XML file.
...
`mem_channel` has no impact on the RTL generated by the compiler.
