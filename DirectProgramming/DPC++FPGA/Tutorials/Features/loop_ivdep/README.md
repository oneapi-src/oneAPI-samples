...
### Specifying that memory accesses to a specific array or pointer do *not* cause loop-carried dependencies
...
The `ivdep` attribute can also be applied to a loop without a specific array or pointer argument. When applied in this manner, the `ivdep` attribute indicates to the compiler that it can ignore all assumed loop-carried dependencies on accesses to all arrays and pointers within the loop. 
...
### Specifying that memory accesses to a specific array or pointer do *not* cause loop-carried dependencies across a fixed distance
