...
Once you have the main report page open, click on the dropdown titled "Throughput Analysis" and choose the "Loop Analysis" option. Now you can choose to look at "Kernel: Kernel<0>" and this will provide information for the three loops of Kernel<0>. Specifically, you want to be looking at the concurrency value found by clicking on the array `a` in the "Bottlenecks" section of the report. In the details pane it will now highlight that the concurrency value for `a` is 3 in addition to the fact that there is a throughput bottleneck on Loop "B3" that has a latency of 222.0. Using this information combined with the knowledge that the "Block Scheduled fMAX" is 240.00, we can make an analytical estimate that using `private_copies` of 3 or higher will deliver optimal throughput performance. 
